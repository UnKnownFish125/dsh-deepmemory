...
> 版本：contract-lib-v0.1 · 2026-08-31 · 移交 deepmemory 维护方实现
...
| 注入分组 | plugin-v3.js 按 type 分组（规则与偏好/决定与目标/事实）+ 保底加权（preference≥0.7 +0.35） | plugin-v3.js:202 |
...
| 方法/路径 | 行为 |
|---|---|
| `GET /v1/memories/search`（POST 同名） | 请求体加 `library`（精确，缺省=全部）、`include_archived`（bool，缺省 false）；**默认排除 storage_tier='archive'** |
...
| `POST /v1/memories/archive` | 归档：`{ids?: [id], library?: str, topic?: str, reason: str, actor}`——按 ids 或 库+topic 批量置 `storage_tier='archive'`（走 migrate 语义）+ 写 `memory_archives` 摘要；**bias 库拒绝归档（403）** |
| `POST /v1/memories/restore` | 恢复：`{id, reason}`（archive→active，沿用 restore_memory） |
...
- 新增独立小节「总行为约束」（`injection.inject_group='category'` 下置于最前）：查询 `library='bias' AND scope='global'`，**每会话开始生成一次、会话内冻结**（内容低频变动，天然满足前缀稳定）；现有"规则与偏好"组保留但改从 bias 库取
...
1. **G1 修复**：`migrate_memory` 到 archive 的条目，`search` 默认**不返回**；`include_archived=true` 返回且带 archive 标记；`restore` 后重新可召回
2. **G2 分库**：`library='core'` 写入"架构决策"、`library='project'` 写入"任务看板"——`search(library='core')` 不含 project 记忆；`search` 不带 library 返回全部（向后兼容）
3. **G4 bias**：写入 `library='bias'` 的记忆在**新会话**注入段「总行为约束」可见；`POST /v1/memories/archive {library:'bias'}` 返回 403；bias 且 scope≠global 的写入返回 400
...
5. **G3 库级归档**：`POST /v1/memories/archive {library:'project', topic:'任务看板'}` 批量归档 → `search` 默认不含、`kb_browse(project)` 显示已归档计数、`restore` 单条可恢复
6. **回归**：6230 health 正常；无 token 401 / 带 Origin 403 / 根路径 404 行为不变；既有 32 个单测全绿 + 新增契约单测（library 校验/归档过滤/for-doc/恢复）
...
分工建议（如交子代理）：SA-A 存储+DDL（G1/G2 表与触发器）、SA-B API（archive/for-doc/libraries/doc-link）、SA-C 插件（注入段/抽取/工具）、SA-D 测试与回归——**接口以 §4 为 frozen 契约**，各 SA 独立可测。
...
- **代码基**：`/www/deepmemory-v063-deploy/memory-server/{server.py, v2_domain.py, config_schema.json}` + `/www/deepseek harness workspace/harness-memory-archive/agent-preset/memory-plugin/plugin-v3.js`
- **派生参照**：`dsh-literatum` 契约 v0.2（同源哲学，接口风格一致）
- **边界**：不重建 schema、不迁移既有数据、不动 6230/6240 端口与鉴权模式；实现顺序 P0→P1→P2，P0 验收过即可先行合入
